...
export default function Config({name, columns}) {
    ...
    const onSubmit = data => {
        database.write(database.TABLE_PAGINATION, name, data);
        setOpen(false);
        ...
        <>
...

            {
                !isOpen
                    ? null
                    : (
                        <div className="bg-light fixed z-50 w-96 h-96 top-3 m-auto p-5 shadow"
                                ...
                                <form onSubmit={handleSubmit(onSubmit)}>
                                    ...
                                    <div className="input-group justify-center">
                                        ...
                                        <input
                                            ...
                                            {...register('numberLineByPage')}
                                        />
                                    </div>

                                    <div className="input-group justify-center">
                                        <label className="font-bold">Hide column</label>
                                        <select
                                            className="input-select w-full"
                                            multiple
                                            defaultValue={config.hiddenColumn}
                                            {...register('hiddenColumn')}
                                        >
                                            <option></option>
                                            {
                                                columns.map(column => (
